...
use crate::dex::{
    ...
    write_uleb128, write_uleb128p1, write_x,
};
...
pub struct DebugInfo {
    pub line_start: u32,
    pub parameter_names: Vec<u32>, // todo: Byte code follows
}
    ...
    pub fn read(
        ...
        for _ in 0..direct_method_size {
            ...
            let code = if code_offset > 0 {
                Some(CodeItem::read(bytes, &mut code_offset)?)
            } else {
                None
            };
...
impl ClassDefItem {
    pub fn read(bytes: &[u8], ix: &mut usize) -> Result<ClassDefItem, DexError> {
        ...
        let class_data = if class_data_offset > 0 {
            Some(ClassDataItem::read(bytes, &mut class_data_offset)?)
        } else {
            None
        };
